...
export function renderProp(field: FieldDescriptor, optionalMode: boolean): string {
  ...
  const decorator = optionalMode || !field.isRequired ? 'ApiPropertyOptional' : 'ApiProperty'
  const metaParts = [
